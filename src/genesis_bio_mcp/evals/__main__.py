"""CLI entrypoint for the eval harness.
    ...
    uv run python -m genesis_bio_mcp.evals [--agentic] [--baseline] [--out report.md] [--filter CATEGORY]
...
(tools-on minus tools-off): the headline "do the tools actually help?" number. Prints the
Markdown report; ``--out`` also writes it plus a sibling ``.json``.
"""
