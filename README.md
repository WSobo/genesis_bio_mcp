...
An MCP server that gives AI agents structured access to **42 tools** across
...
42 tools across 24 data sources (plus local RDKit cheminformatics, the UMA-Inverse model service, and an optional embedding-backed corpus store), organized into 10 categories. Full details
...
| **Corpus (indexed)** | `corpus_describe`, `corpus_search_targets_by_sequence`, `corpus_find_similar_compounds`, `corpus_search_compounds` *(v0.6.0; optional)* | Postgres + pgvector over a curated bioactivity corpus |
