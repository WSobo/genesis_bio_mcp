...
An MCP server that gives AI agents structured access to **40 tools** across
...
| Doc | What it covers |
|---|---|
| **[docs/tools.md](docs/tools.md)** | Full catalog of all 40 tools grouped by category, input fields, and use cases |
...
40 tools across 24 data sources (plus local RDKit cheminformatics, the UMA-Inverse model service, and an optional embedding-backed corpus store), organized into 10 categories. Full details
...
| **Druggability** | `get_compounds`, `compute_molecular_properties`, `batch_compute_molecular_properties`, `standardize_structure`, `search_similar_compounds`, `get_chembl_compounds` | PubChem, RDKit, ChEMBL |
...
| **Corpus (indexed)** | `corpus_describe`, `corpus_search_targets_by_sequence` *(v0.6.0; more `corpus_*` tools in progress)* | Postgres + pgvector over a curated bioactivity corpus (optional) |
...
```bash
uv sync
uv run pytest tests/ -v          # full unit + integration test suite
