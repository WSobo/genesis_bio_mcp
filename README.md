...
An MCP server that gives AI agents structured access to **41 tools** across
...
| Doc | What it covers |
|---|---|
| **[docs/tools.md](docs/tools.md)** | Full catalog of all 41 tools grouped by category, input fields, and use cases |
...
41 tools across 24 data sources (plus local RDKit cheminformatics, the UMA-Inverse model service, and an optional embedding-backed corpus store), organized into 10 categories. Full details
...
| **Corpus (indexed)** | `corpus_describe`, `corpus_search_targets_by_sequence`, `corpus_find_similar_compounds` *(v0.6.0; optional)* | Postgres + pgvector over a curated bioactivity corpus |
